...
"You - you alone will have the stars as no one else has themIn one of the stars I shall be living. In one of them I shall be laughing. And so it will be as if all the stars were laughing, when you look at the sky at night...You - only you - will have stars that can laugh.",
	...
	for dirpath, dirs, files in os.walk("fortune1"):
		for single_file in files:
			if fnmatch.fnmatch(single_file, "*txt"):
				filepathtemp=os.path.abspath(single_file);
				f = open(os.path.join(dirpath, single_file)) 
				filecontenttemp=f.read();
